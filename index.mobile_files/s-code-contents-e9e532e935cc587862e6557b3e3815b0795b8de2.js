var s_account=_satellite.getVar("site: report suite id");
if (s_account.indexOf('preprod')>=0||s_account.indexOf('gamma')>=0){
  s_account+=",audibletestglobal";
}
else{
  s_account+=",audibleglobal";
}
var s=s_gi(s_account)
s.visitorNamespace="audible";
_satellite.notify((document.domain.indexOf('.co.uk')>=0),3);
if(document.domain.indexOf('.co.uk')>=0||document.domain.indexOf('.com.au')>=0||document.domain.indexOf('.co.jp')>=0){
...
if(s.Util.getQueryParam('versionCode'))
	s.eVar64=s.Util.getQueryParam('versionCode');
if(s.Util.getQueryParam('versionName'))
	s.eVar65=s.Util.getQueryParam('versionName');
if(s.Util.getQueryParam('androidBuildBrand'))
	s.eVar66=s.Util.getQueryParam('androidBuildBrand');
if(s.Util.getQueryParam('androidBuildDevice'))
	s.eVar67=s.Util.getQueryParam('androidBuildDevice');
if(s.Util.getQueryParam('androidBuildDisplay'))
	s.eVar68=s.Util.getQueryParam('androidBuildDisplay');
if(s.Util.getQueryParam('androidBuildModel'))
	s.eVar69=s.Util.getQueryParam('androidBuildModel');
if(s.Util.getQueryParam('androidBuildHost'))
	s.eVar70=s.Util.getQueryParam('androidBuildHost');
if(s.Util.getQueryParam('androidBuildVersionSDK'))
	s.eVar71=s.Util.getQueryParam('androidBuildVersionSDK');
if(s.Util.getQueryParam('iosBuildBrand'))
	s.eVar66=s.Util.getQueryParam('iosBuildBrand');
if(s.Util.getQueryParam('iosBuildDevice'))
	s.eVar67=s.Util.getQueryParam('iosBuildDevice');
if(s.Util.getQueryParam('iosBuildDisplay'))
	s.eVar68=s.Util.getQueryParam('iosBuildDisplay');
if(s.Util.getQueryParam('iosBuildModel'))
	s.eVar69=s.Util.getQueryParam('iosBuildModel');
if(s.Util.getQueryParam('iosBuildHost'))
	s.eVar70=s.Util.getQueryParam('iosBuildHost');
if(s.Util.getQueryParam('iosBuildVersionSDK'))
	s.eVar71=s.Util.getQueryParam('iosBuildVersionSDK');


/********************** PLUGIN CONFIGURATION****************************/
/* Plugin Config */
s.usePlugins=true
_satellite.notify('test out',3);
function s_doPlugins(s) {
  _satellite.notify('test in',3);
	// ECID Data
	s.contextData['ecid.pageDomain'] = location.hostname;
	s.contextData['ecid.version'] = (typeof(Visitor) != "undefined" ? s.visitor.version : "not set");
	s.contextData['ecid.aamVersion'] = (typeof(DIL) != "undefined" ? DIL.version : "not set");
	s.contextData['ecid.atVersion'] = (typeof(adobe) != "undefined" ? adobe.target.VERSION : "not set");
	if (typeof(Visitor) != "undefined") {
		...
		if (icsmcvid===true) {s.contextData['ecid.icsmcvid'] = "client side mid";}
		else if (icsmcvid===false) {s.contextData['ecid.icsmcvid'] = "server side mid";}
		if (mcidcto===true) {s.contextData['ecid.mcidcto'] = "request timed out";}
		else if (mcidcto===false) {s.contextData['ecid.mcidcto'] = "successful response";}
		if (aidcto===true) {s.contextData['ecid.aidcto'] = "request timed out";}
		else if (aidcto===false) {s.contextData['ecid.aidcto'] = "successful response";}
		if (aamidcto===true) {s.contextData['ecid.aamidcto'] = "request timed out";}
		else if (aamidcto===false) {s.contextData['ecid.aamidcto'] = "successful response";}
		s.contextData['ecid.mcorgid'] = "D=mcorgid";
	}
	var ecidContext =
	"contextData.ecid.pageDomain,contextData.ecid.version,contextData.ecid.atVersion,contextData.ecid.aamVersion,contextData.mcid.mcorgid";
	var ltv = s.linkTrackVars;
	if (ltv==="None") {s.linkTrackVars = ecidContext} else {s.linkTrackVars = s.linkTrackVars+','+ecidContext}
	...
	s.eVar91=s.prop45=s.getPreviousValue(s.pageName,'gpv_pn');
	s.prop44=s.getPercentPageViewed();
	s.eVar97=s.prop46=s_getLoadTime();
	s.eVar99=s.getNewRepeat(30, 's_getNewRepeat');
	s.prop42=s.getDaysSinceLastVisit('s_lv');
	
  //s.eVar8=s.prop43=s.getTimeParting('h','-5');
  //Upated Time Parting Variable January 2020
  s.eVar8= s.prop43 = getTimeParting("America/New_York").split("|")[4].split("=")[1];
  
	s.campaign = s.getValOnce(s.Util.getQueryParam("cid"),'s_cid',0);
	//s.eVar34 = s.getValOnce(s.Util.getQueryParam("intcid"),'s_intcid',0);
...
//Excluding Actiivity Map Sign Out - WB 01/2020
//s.ActivityMap.regionExclusions = "navTop_sl0_popover"; 
s.ActivityMap.linkExclusions = "Sign out";
...
/* Adobe Consulting Plugin: getTimeParting v6.2 January 2020 */
...
/* Plugins Section */
/* Cookie Combining Utility v1.1 (Minified) */
if(typeof cookieCombiningUtility==="undefined"){var cookieCombiningUtility="true";var c_rspers=function(){var s=this,cv=s.c_rr("s_pers"),date=(new Date).getTime(),expd=null,cvarr=[],vcv="";if(!cv)return"";cvarr=cv.split(";");for(var i=0,l=cvarr.length;i<l;i++){expd=cvarr[i].match(/\|([0-9]+)$/);if(expd&&parseInt(expd[1])>=date)vcv+=cvarr[i]+";"}return vcv};var c_r=function(k){var s=this,d=new Date,v=s.c_rr(k),c=s.c_rspers(),i,m,e;if(v)return v;k=s.escape?s.escape(k):encodeURIComponent(k);i=c.indexOf(" "+
k+"=");c=i<0?s.c_rr("s_sess"):c;i=c.indexOf(" "+k+"=");if(i<0)return"";m=c.indexOf("|",i);e=c.indexOf(";",i);m=m>0?m:e;return s.unescape?s.unescape(c.substring(i+2+k.length,m<0?c.length:m)):decodeURIComponent(c.substring(i+2+k.length,m<0?c.length:m))};var c_w=function(k,v,e){var s=this,d=new Date(0),ht=0,pn="s_pers",sn="s_sess",pc=0,sc=0,pv,sv,c,i,t,f;if(k==="s_sq"){s.c_wr(k,v);return}if(s.c_rr(k))s.c_wr(k,"",d);k=s.escape?s.escape(k):encodeURIComponent(k);pv=s.c_rspers();i=pv.indexOf(" "+k+"=");
if(i>-1){pv=pv.substring(0,i)+pv.substring(pv.indexOf(";",i)+1);pc=1}sv=s.c_rr(sn);i=sv.indexOf(" "+k+"=");if(i>-1){sv=sv.substring(0,i)+sv.substring(sv.indexOf(";",i)+1);sc=1}d=new Date;if(e){if(e===1)e=new Date,f=e.getYear(),e.setYear(f+5+(f<1900?1900:0));if(e.getTime()>d.getTime()){pv+=" "+k+"="+(s.escape?s.escape(v):encodeURIComponent(v))+"|"+e.getTime()+";";pc=1}}else{sv+=" "+k+"="+(s.escape?s.escape(v):encodeURIComponent(v))+";";sc=1}sv=sv.replace(/%00/g,"");pv=pv.replace(/%00/g,"");if(sc)s.c_wr(sn,
sv,0);if(pc){t=pv;while(t&&t.indexOf(";")>-1){var t1=parseInt(t.substring(t.indexOf("|")+1,t.indexOf(";")));t=t.substring(t.indexOf(";")+1);ht=ht<t1?t1:ht}d.setTime(ht);s.c_wr(pn,pv,d)}return v===s.c_r(s.unescape?s.unescape(k):decodeURIComponent(k))};var c_d=function(c){var s=this;var d=new Date(0);s.c_w(c,"",d);return};s.ccuSetup=function(){var s=this;if(!s.c_wr)s.c_wr=s.c_w;if(!s.c_rr)s.c_rr=s.c_r;s.c_rspers=c_rspers;s.c_r=s.cookieRead=c_r;s.c_w=s.cookieWrite=c_w;s.c_d=c_d};s.ccuSetup()};
/*
